...
export const TowerHover = () => {
	const [activeTower, setActiveTower] = useState<number>(0)
	const componentRef = useRef<HTMLDivElement>(null)

	const towerDescription: { title: string; description: string }[] = [
		{ title: 'ЖК Сокольники', description: 'Классные башенки, покупай пзязя' },
		{ title: 'Башня 1', description: 'Ваще норм вид' },
		{ title: 'Башня 2', description: '2 лапомойки: для животного и для его собачки' },
		{ title: 'Башня 3', description: 'Дешевые квартирки' },
	]

	useOutsideClick(componentRef, () => setActiveTower(0))

	return (
		<div className={s.root} id='towerHover'>
			<div className={s.imageWrapper}>
				...
				/>
				<div
					className={classNames(s.wrapper, s.wrapperMobile)}
					ref={componentRef}
					onClick={() => setActiveTower(0)}>
					<svg
						width='100%'
						height='100%'
						...
						<path
							onClick={(e) => {
								e.stopPropagation(), setActiveTower(1)
							}}
						...
						<path
							onClick={(e) => {
								e.stopPropagation(), setActiveTower(2)
							}}
						...
						<path
							onClick={(e) => {
								e.stopPropagation(), setActiveTower(3)
							}}
			...
			<div className={s.textWrappers}>
				<div className={s.title}>{towerDescription[activeTower].title}</div>
				<div className={s.description}>{towerDescription[activeTower].description}</div>
			</div>
		</div>
	)
}
